...
public class APIOfficeVisitTest {
    ...
    public void testPreScheduledOfficeVisit () throws Exception {
        ...
        final ICDCodeForm icdF = new ICDCodeForm();
        icdF.setCode( "E78.0" );
        icdF.setName( "cholesterolemia" );

        final ICDCode icd = new ICDCode( icdF );
        // Post this newly created icd code
        ...
        // See that ICD codes can also be gotten
    ...
    /**
     * Tests OfficeVisitAPI with focus on patient functionality
     *
     * @throws Exception
     *
     */
    ...
    public void testOfficeVisitAPIPatient () throws Exception {
        ...
        // Find own office visits while logged in as the patient
        mvc.perform( get( "/api/v1/officevisits/myofficevisits" ).with( user( "patient" ).password( "123456" ) ) )
        ...
        // Testing the marker for the system to know that Patient has viewed the
        // visit
        mvc.perform( post( "/api/v1/officevisits/patient/view/" + id ).with( user( "patient" ).password( "123456" ) )
                .contentType( MediaType.APPLICATION_JSON ).content( TestUtils.asJsonString( visit ) ) )
                .andExpect( status().isOk() );
    }

    /**
     * Tests OfficeVisitAPI with focus on HCP functionality
     *
     * @throws Exception
     *
     */
    ...
    public void testOfficeVisitAPIHCP () throws Exception {
        ...
        // Testing the marker for the system to know that HCP has viewed the
        // visit
        mvc.perform( post( "/api/v1/officevisits/hcp/view/" + id ).with( user( "hcp" ).password( "123456" ) )
                .contentType( MediaType.APPLICATION_JSON ).content( TestUtils.asJsonString( visit ) ) )
                .andExpect( status().isOk() );
    }
}
